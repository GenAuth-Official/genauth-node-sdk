...
function getCanonicalizedHeaders(headers: { [x: string]: any }) {
  ...
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i];
    if (key.startsWith("x-genauth-") || key === "date") {
...
export function buildAuthorization(
  ...
  return `GenAuth ${accessKeyId}:${buildSignature(
    accessKeySecret,
    stringToSign
  )}`;
}
...
const DEFAULT_UA =
  `GenAuthIdentityCloud (${os.platform()}; ${os.arch()}) ` +
...
export const DEFAULT_HEADERS = () => {
  return {
    ...
    "x-genauth-signature-nonce": makeNonce(),
    "x-genauth-signature-method": "HMAC-SHA1",
    "x-genauth-signature-version": "1.0",
    "user-agent": DEFAULT_UA,
    "x-genauth-sdk-version": `gen-auth-node-sdk:${pkg.version}`,
  };
};
